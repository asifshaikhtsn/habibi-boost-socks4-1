...
async def main():
    print(f"[Boost-{PROTO} 0] Starting ProxRipper {PROTO.upper()} booster FIRST 50k...")
    ...
    print(f"[Boost-{PROTO}] Fetching ProxRipper {PROTO.upper()} FIRST 50k ({SKIP_FIRST}-{SKIP_FIRST+MAX_PROXIES})...")
    text = ""
    try:
    ...
    print(f"[Boost-{PROTO}] Fetched {len(all_proxies)} total unique, using FIRST 50k: {SKIP_FIRST}-{SKIP_FIRST+len(proxies)} ({len(proxies)} proxies)")
